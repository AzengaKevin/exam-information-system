...
{    
    ...
    /** @group exam-scores */
    public function testAuthorizedTecherCanPublishTopExamStudentSubjectWise()
    {
        $this->withoutExceptionHandling();
        ...
        // Associate Teacher and Responsibility
        $this->teacher->responsibilities()->attach($responsibility, ['level_id' => $levelUnit->level->id]);
        ...
        // Act
        $response = Livewire::test(LevelExamScores::class, ['exam' => $exam, 'level' => $levelUnit->level])
            ->call('publishTopStudentsSubjectWise');

        $dbDriver = DB::connection()->getPdo()->getAttribute(\PDO::ATTR_DRIVER_NAME);

        if ($dbDriver == 'mysql') {
            $this->assertTrue(DB::table('exam_level_top_students_per_subject')->count() > 0);
        } else {
